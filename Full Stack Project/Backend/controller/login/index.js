...
async function createUser(req, res) {
    ...
    try {
        const existUser = await User.findOne({email: email, is_deleted: false});
        
        if(existUser)
            return res.status(statusCode.ALREADY_USER_EXISTS).send({ message: 'user already exists'});

        const deletedUser = await User.findOne({email: email, is_deleted: true});

        if(deletedUser)
            return res.status(statusCode.ALREADY_USER_EXISTS).send({ message: 'this email user was already exists but account is deleted'});
        ...
        res.status(statusCode.OK).send({ message: 'Your account is registered' });
    } catch (error) {
        console.log('Fetching error at creating user', error);
        res.status(statusCode.SERVER_ERROR).send({ message: 'Server Error' });
    }
}
